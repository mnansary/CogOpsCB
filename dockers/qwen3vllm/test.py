...
"""
    ...
    def invoke_with_tools(
        ...
        try:
            ...
            # --- FIX FOR QWEN3 TOOL CALL FORMAT ---
            # Manually parse the <tool_call> string if the standard tool_calls object is empty.
            if not tool_calls and response_message.content and "<tool_call>" in response_message.content:
                print("   [Detected non-standard tool call format. Manually parsing...]")
                # Extract the JSON part of the tool call string
                call_str = response_message.content
                json_start = call_str.find('{')
                json_end = call_str.rfind('}') + 1
                if json_start != -1 and json_end != -1:
                    tool_json_str = call_str[json_start:json_end]
                    tool_data = json.loads(tool_json_str)
                    
                    # Reconstruct a pseudo tool_calls object that our code can use
                    from types import SimpleNamespace
                    tool_calls = [
                        SimpleNamespace(
                            id=f"call_manual_{tool_data['name']}",
