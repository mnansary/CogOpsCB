...
from typing import Any, Type, TypeVar, AsyncGenerator, List
...
async def main():
    ...
    # Load Small LLM Config
    small_api_key = os.getenv("VLLM_SMALL_API_KEY")
    small_model = os.getenv("VLLM_SMALL_MODEL_NAME")
    small_base_url = os.getenv("VLLM_SMALL_BASE_URL")
    small_llm_service = None
    if all([small_api_key, small_model, small_base_url]):
        small_llm_service = AsyncLLMService(small_api_key, small_model, small_base_url, max_context_tokens=32768)
    else:
        print("\nWARNING: Small LLM environment variables not set. Skipping tests for small model.")

    # Load Medium LLM Config
    medium_api_key = os.getenv("VLLM_MEDIUM_API_KEY")
    medium_model = os.getenv("VLLM_MEDIUM_MODEL_NAME")
    medium_base_url = os.getenv("VLLM_MEDIUM_BASE_URL")
    medium_llm_service = None
    if all([medium_api_key, medium_model, medium_base_url]):
        medium_llm_service = AsyncLLMService(medium_api_key, medium_model, medium_base_url, max_context_tokens=131072)
    else:
        print("\nWARNING: Medium LLM environment variables not set. Skipping tests for medium model.")

    # --- Test Cases ---

    # Test 1: Small LLM - Invoke
    if small_llm_service:
        print("\n--- Test 1: Small LLM - Invoke (Async) ---")
        try:
            prompt = "জন্ম নিবন্ধন সনদের গুরুত্ব কী?"
            print(f"Prompt: {prompt}")
            response = await small_llm_service.invoke(prompt, temperature=0.1, max_tokens=256)
            print(f"Response:\n{response}")
        except Exception as e:
    ...
    if small_llm_service:
        print("\n--- Test 2: Small LLM - Stream (Async) ---")
        try:
            prompt = "পাসপোর্ট অফিসের একজন কর্মকর্তার একটি সংক্ষিপ্ত বর্ণনা দিন।"
            print(f"Prompt: {prompt}\nStreaming Response:")
            async for chunk in small_llm_service.stream(prompt, temperature=0.2, max_tokens=256):
    ...
    if small_llm_service:
        print("\n--- Test 3: Small LLM - Structured Invoke (Async) ---")
        try:
            prompt = "আমার নাম 'করিম চৌধুরী', পিতার নাম 'রহিম চৌধুরী', আমি একজন ছাত্র। এই তথ্য দিয়ে একটি এনআইডি কার্ডের তথ্য তৈরি করুন।"
            print(f"Prompt: {prompt}")
            nid_data = await small_llm_service.invoke_structured(prompt, NIDInfo, temperature=0.0)
            print(f"Parsed Response:\n{nid_data.model_dump_json(indent=2 )}")
        except Exception as e:
            print(f"An error occurred: {e}")
            
    # Test 4: Medium LLM - Invoke
    if medium_llm_service:
        print("\n--- Test 4: Medium LLM - Invoke (Async) ---")
        try:
            prompt = "বাংলাদেশের ভূমি উন্নয়ন কর পরিশোধ করার বিভিন্ন উপায় কী কী?"
            print(f"Prompt: {prompt}")
            response = await medium_llm_service.invoke(prompt, temperature=0.1, max_tokens=512)
            print(f"Response:\n{response}")
        except Exception as e:
            print(f"An error occurred: {e}")

    # Test 5: Medium LLM - Stream
    if medium_llm_service:
        print("\n--- Test 5: Medium LLM - Stream (Async) ---")
        try:
            prompt = "ই-পাসপোর্ট এবং এমআরপি পাসপোর্টের মধ্যে প্রধান পার্থক্যগুলো একটি তালিকা আকারে লিখুন।"
            print(f"Prompt: {prompt}\nStreaming Response:")
            async for chunk in medium_llm_service.stream(prompt, temperature=0.1, max_tokens=512):
                print(chunk, end="", flush=True)
            print()
        except Exception as e:
            print(f"An error occurred: {e}")

    # Test 6: Medium LLM - Structured Invoke
    if medium_llm_service:
        print("\n--- Test 6: Medium LLM - Structured Invoke (Async) ---")
        try:
            prompt = "জরুরি ভিত্তিতে একটি নতুন ই-পাসপোর্ট আবেদনের জন্য প্রয়োজনীয় তথ্য দিন। এর মেয়াদ হবে ৫ বছর।"
            print(f"Prompt: {prompt}")
            passport_data = await medium_llm_service.invoke_structured(prompt, PassportInfo, temperature=0.0)
            print(f"Parsed Response:\n{passport_data.model_dump_json(indent=2 )}")
        except Exception as e:
            print(f"An error occurred: {e}")
