...
def initialize_embedder(triton_url: str) -> GemmaTritonEmbedder:
    """Initializes the Gemma Triton embedder."""
...
def run_semantic_search(collection: chromadb.Collection, embedder: GemmaTritonEmbedder, query_text: str, n_results: int = 3):
    """
    ...
    try:
        # Embed the query using the query-specific method (asymmetric embedding)
        query_embedding = embedder.embed_queries([query_text])[0]

        # The core query operation in ChromaDB.
        # Pass the pre-computed query embedding instead of text.
...
def main():
    ...
    try:
        ...
        for category, query in test_queries.items():
            ...
            run_semantic_search(collection, embedder, query, n_results=3)
